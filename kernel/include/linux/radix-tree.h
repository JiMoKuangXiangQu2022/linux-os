...
struct radix_tree_node {
	/* 当前节点slots[]存放item的索引移位：即 (index>>RADIX_TREE_MAP_SHIFT) & RADIX_TREE_MAP_MASK */
	unsigned char	shift;		/* Bits remaining in each slot */
	unsigned char	offset;		/* Slot offset in parent (当前节点指针存放在父节点@parent slots[]中的偏移位置) */
	unsigned char	count;		/* Total entry count (节点上 @slots[] 中已存放 item 数目) */
...
struct radix_tree_root {
	...
	struct radix_tree_node	__rcu *rnode; /* 存储第1个数据 item 或 根节点 radix_tree_node 指针 */
};
