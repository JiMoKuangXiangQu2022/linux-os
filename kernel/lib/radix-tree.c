 ...
 * @parent: 新创建节点的父级节点
 * @shift:  新节点的索引移位
 * @offset: 新节点的指针存放在父级节点 slots[] 中的 offset 偏移处
 * @count : 新节点中初始使用的 slots 数目
 */ 
 ...
 * 加载【根节点】信息：
 * . 根节点指针：从 @nodep 返回；
 * . 根节点最大索引值：从 @maxindex 返回；
 * 返回值：【 根节点索引移位 + RADIX_TREE_MAP_SHIFT：从该值计算根节点索引区间范围 】。
 */
 ...
 */
/* 
 * 扩展基树，直到其索引区间能容纳 @index 。 
 * 具体是创建新的 radix_tree_node ，将基树增高，让新的 radix_tree_node
 * 成为基树新的根节点，旧的根节点及其子树将成为新的根节点的子树。
 * 返回值：基树新根节点的索引移位 + RADIX_TREE_MAP_SHIFT .
 */ 
...
/*
 * 层层压缩基树：
 * 将那些只有一个最左子节点的根节点删除：它们的索引域为0，没有存在的必要了.
 * 返回值：如果压缩过基树（删除过根节点），返回 true ；否则返回 false 。
 */ 
			...
			void __rcu ***slotp)
{
	...
	while (shift > order) {
		/* 
		 * 基树节点层级向下移动一级。
		 * 每 RADIX_TREE_MAP_SHIFT 为一级的索引，更高位表示更高层级的索引：
		 *                                    index
		 *   --------------------------------------------------------------------------
		 *  | ... | RADIX_TREE_MAP_SHIFT | RADIX_TREE_MAP_SHIFT | RADIX_TREE_MAP_SHIFT |
		 *   --------------------------------------------------------------------------
		 */
